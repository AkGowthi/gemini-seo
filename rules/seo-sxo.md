...
- Fetch the target URL using `"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run render_page.py "<url>" --mode auto --json` (SPA-aware SSRF-protected renderer)
- Parse with `"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run parse_html.py --url "<url>"` to extract SEO elements
...
- [ ] URL was fetched via `"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run render_page.py --mode auto` (not raw curl)
