...
1. Check credentials: `"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run google_auth.py --check --json`
2. Determine tier (0 = API key, 1 = + service account, 2 = + GA4)
3. Execute tier-appropriate analysis
4. Format output to match gemini-seo conventions

## Tier-Based Workflow

### Tier 0 (API Key Only)
- Run PSI + CrUX on homepage: `"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run pagespeed_check.py <url> --json`
- Run CrUX History for origin: `"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run crux_history.py <origin> --origin --json`
- Report CWV field data with traffic-light ratings

### Tier 1 (+ Service Account)
- All Tier 0 checks
- GSC top queries/pages (28 days): `"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run gsc_query.py --property <prop> --json`
...
- URL Inspection on homepage + key pages: `"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run gsc_inspect.py <url> --json`
- GSC sitemap status: `"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run gsc_query.py sitemaps --property <prop> --json`

### Tier 2 (Full)
- All Tier 1 checks
- GA4 organic traffic (28 days): `"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run ga4_report.py --property <id> --json`
- Top organic landing pages: `"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run ga4_report.py --property <id> --report top-pages --json`
...
Match existing gemini-seo patterns:
...
```bash
"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run google_report.py --type full --data data.json --domain DOMAIN --format pdf --json
```
...
- Generated PDF/HTML/XLSX reports under `output_dir/` with `"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run google_report.py --output-dir "$output_dir"`
