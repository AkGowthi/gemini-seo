...
2. Analyze product pages with `"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run render_page.py <URL> --mode auto` and `"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run parse_html.py <URL>`
...
4. If DataForSEO credentials available, fetch marketplace data via
   `"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run dataforseo_merchant.py`

## Cost Guardrails

Before ANY DataForSEO Merchant API call:
```bash
"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run dataforseo_costs.py check <endpoint>
```
...
```bash
"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run dataforseo_costs.py log <endpoint> <actual_cost>
```
