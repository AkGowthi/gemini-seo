...
3. Name it (e.g., "Gemini SEO") and note the project ID
...
2. Name: `gemini-seo` (or similar)
3. Skip optional permissions steps
4. Click on the created service account > **Keys > Add Key > Create new key > JSON**
5. Download the JSON file and store it securely (e.g., `~/.config/gemini-seo/service_account.json`)
...
```json
{
  "service_account_path": "~/.config/gemini-seo/service_account.json",
...
```bash
"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run google_auth.py --check
```
