...
1. Fetch the target URL via `"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run render_page.py <URL> --mode auto` (SPA-aware and SSRF-safe)
2. Parse with `"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run parse_html.py <URL>` to extract: title, H1, meta description,
...
- [ ] Target URL was fetched via `"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run render_page.py <URL> --mode auto` (not raw curl/fetch)
