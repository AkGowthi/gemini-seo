...
This is the canonical methodology gemini-seo uses to assemble raw findings
...
- Raw HTML + rendered HTML (via `"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run render_page.py`)
...
- Core Web Vitals field data from CrUX (via `"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run pagespeed_check.py`)
...
- A PDF via `"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run google_report.py` when the user asks for one.
...
- Surface what gemini-seo itself **could not measure** (offline
...
These principles are gemini-seo's; they are not the user's. Surface them
for the user when:
