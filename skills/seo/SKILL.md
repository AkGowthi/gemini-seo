...
`"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run <script.py>`. Set
`GEMINI_SEO_ROOT` to this repository's root when running inside a Gemini /
Antigravity workspace, or run `./scripts/gemini-seo` directly from a clone.
The launcher is found without any `PATH` entry; this repository ships no
top-level `bin/`. Never invoke bundled scripts with a bare Python
interpreter.
...
`"${GEMINI_SEO_ROOT}/scripts/gemini-seo" setup`, report core and Chromium
status separately, and do not fall back to global or user package installation.
For diagnosis, execute
`"${GEMINI_SEO_ROOT}/scripts/gemini-seo" doctor --json`; its output
intentionally omits absolute paths and environment values. If any
`"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run` command reports that setup is
required, suggest `/seo setup` and do not improvise a `pip install`.

## Orchestration Logic

When the user invokes `/seo audit`, run the specialist analyses sequentially
(load the matching `rules/seo-*.md` for each specialist's focus, and run each
one inline before moving to the next):
1. Detect business type (SaaS, local, ecommerce, publisher, agency, other)
2. Analyze seo-technical, seo-content, seo-schema, seo-sitemap, seo-performance, seo-visual, seo-geo
3. If Google API credentials detected (`"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run google_auth.py --check`), also run seo-google analysis
4. If local business detected, also run seo-local analysis
5. If local business detected AND DataForSEO MCP available, also run seo-maps analysis
6. If backlink APIs detected (`"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run backlinks_auth.py --check`), also run seo-backlinks analysis
7. If a site-crawling MCP (e.g. Firecrawl) is available, use it to discover all site URLs before analysis
8. If content strategy signals detected (blog, pillar pages, topic clusters), also run seo-cluster analysis
9. If e-commerce detected, also run seo-ecommerce analysis
10. If drift baseline exists for this URL (`"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run drift_history.py <url>`), also run seo-drift analysis
...
After any analysis command completes, offer to generate a PDF report via `"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run google_report.py`.

## Synthesis Methodology

Audits are not just findings, they are findings synthesized into a coherent
strategy. gemini-seo uses a 10-principle thinking framework grouped into four
...
The extension mirrors above (`seo-dataforseo`, `seo-image-gen`) ship in
`skills/`. The upstream project offers additional MCP extensions (Firecrawl,
Ahrefs, SE Ranking, Profound, Bing Webmaster, Unlighthouse); install those
from the upstream [claude-seo](https://github.com/AgriciDaniel/claude-seo)
`extensions/` directory if your harness supports MCP.

## Specialist Analyses (rules/)

Each audit analysis maps to a `rules/seo-*.md` file that clarifies that
specialist's focus and output format. Run them sequentially during audits:
...
- `seo-flow` -- FLOW framework prompts (conditional: used in content strategy workflows)
