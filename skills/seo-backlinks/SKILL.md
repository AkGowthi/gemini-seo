...
2. **Moz API** (free signup): `"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run backlinks_auth.py --check moz --json`
3. **Bing Webmaster** (free signup): `"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run backlinks_auth.py --check bing --json`
4. **Keywords Everywhere** (free signup, single-metric): `"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run backlinks_auth.py --check keywordseverywhere --json`
...
Run `"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run backlinks_auth.py --check --json` to detect all sources at once.
...
**Moz API:** `"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run moz_api.py metrics <url> --json` → Domain Authority, Page Authority, Spam Score, linking root domains, external links.

**Keywords Everywhere:** `"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run keywordseverywhere_api.py rank <domain> --json` → 0-10 domain rank only (no link counts). Use as a fallback when Moz isn't configured; do not use in place of Moz when both are available.

**Common Crawl:** `"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run commoncrawl_graph.py <domain> --json` → PageRank, harmonic centrality, and low-confidence rank/presence data.
...
**Moz API:** `"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run moz_api.py anchors <url> --json`

**Bing Webmaster:** `"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run bing_webmaster.py links <url> --json` (extract anchor text from link details)
...
**Moz API:** `"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run moz_api.py domains <url> --json` → domains with DA scores

**Common Crawl:** `"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run commoncrawl_graph.py <domain> --json` → domain-level rank/presence data, no verified referring-domain counts
...
**Moz API:** Raw vendor spam_score from `"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run moz_api.py metrics <url> --json` (source-label the value; apply thresholds only if verified against current Moz docs)

**Verification Crawler:** `"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run verify_backlinks.py --target <url> --links <file> --json` (verify suspicious links still exist)
...
**Moz API:** `"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run moz_api.py pages <domain> --json`
...
**Bing Webmaster:** `"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run bing_webmaster.py compare <url1> <url2> --json`
...
**Moz API:** Compare DA/PA between domains via `"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run moz_api.py metrics <url> --json` for each
...
**Verification Crawler:** For known links, verify current status with `"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run verify_backlinks.py --target <url> --links <file> --json`
...
```bash
"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run validate_backlink_report.py --report <report>.json --json
```
