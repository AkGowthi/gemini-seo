...
- XML sitemap: run `"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run sitemap_discovery.py <url> --json`; require a
...
"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run agent_ux_check.py https://example.com --json
```
...
`"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run render_page.py <url> --a11y-tree --json`.
...
If Google API credentials are configured, use `"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run pagespeed_check.py <url> --json` for real PSI + CrUX field data (replaces lab-only CWV estimates), `"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run crux_history.py <url> --json` for 25-week CWV trends, and `"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run gsc_inspect.py <url> --json` for real indexation status per URL.
...
To audit a pre-deployment host, the operator names it in `GEMINI_SEO_LOCAL_TARGETS`, a comma-separated list of `host` or `host:port` entries:

```bash
GEMINI_SEO_LOCAL_TARGETS="localhost:3000,127.0.0.1:8080,100.101.102.103" \
  "${GEMINI_SEO_ROOT}/scripts/gemini-seo" run fetch_page.py http://localhost:3000/
```
