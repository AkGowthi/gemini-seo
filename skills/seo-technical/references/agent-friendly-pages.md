...
`"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run render_page.py <URL> --mode auto --a11y-tree --json` loads the
page headlessly and captures Chromium's full accessibility tree through CDP.
Use `"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run agent_ux_check.py <URL> --json` for the bounded Agent-UX
