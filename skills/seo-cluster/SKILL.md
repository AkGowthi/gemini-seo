...
description: >
  ...
  visualizations. Optionally executes content creation if gemini-blog is installed.
...
| `/seo cluster execute` | Execute plan: create content via gemini-blog or output briefs |
...
instead of WebSearch for SERP data. Run `"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run dataforseo_costs.py check serp_organic_live_advanced --count N`
...
### Check for gemini-blog

```
Test: Does ~/.gemini/skills/blog/SKILL.md exist?
```

**If gemini-blog IS installed:**
...
**If gemini-blog is NOT installed:**
...
4. Inform user: "Install [gemini-blog](https://github.com/AgriciDaniel/gemini-blog)
...
| `cluster-briefs/` | Content briefs (if no gemini-blog) |
...
| "gemini-blog not installed" | Execute attempted without blog skill | Generate content briefs instead; suggest installation |
...
- All URLs fetched via `"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run render_page.py <url> --mode auto` (SPA-aware SSRF protection via `url_safety`)
