...
```
~/.cache/gemini-seo/drift/baselines.db
```
...
2. Fetch page via `"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run fetch_page.py <URL>`
3. Parse HTML via `"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run parse_html.py <URL>`
4. Optionally fetch CWV via `"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run pagespeed_check.py <URL>` (use `--skip-cwv` to skip)
5. Hash HTML body and schema content (SHA-256)
6. Store snapshot in SQLite

**Execution:**
```bash
"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run drift_baseline.py <url>
"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run drift_baseline.py <url> --skip-cwv
```
...
**Execution:**
```bash
"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run drift_compare.py <url>
"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run drift_compare.py <url> --baseline-id 5
"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run drift_compare.py <url> --skip-cwv
```
...
```bash
"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run drift_report.py <comparison_json_file> --output drift-report.html
```
...
**Execution:**
```bash
"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run drift_history.py <url>
"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run drift_history.py <url> --limit 10
```
...
- **All URL fetching** goes through `"${GEMINI_SEO_ROOT}/scripts/gemini-seo" run fetch_page.py`, which enforces SSRF protection
