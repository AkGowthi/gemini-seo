...
the standalone AI image generation skill. Gemini SEO ships it as an optional mirror.

This skill has two components with distinct roles:
- **SKILL.md** (this file): Handles interactive `/seo image-gen` commands for generating images
- **Rule** (`rules/seo-image-gen.md`): Audit-only specialist run during `/seo audit` to assess existing OG/social images and produce a generation plan (never auto-generates)
...
| MCP unavailable | Configure MCP with `./extensions/banana/install.sh`; gemini-seo does not vendor a local generation fallback script |
