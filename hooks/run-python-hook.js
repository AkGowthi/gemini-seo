...
function pythonCandidates() {
  const candidates = [];
  if (process.env.GEMINI_SEO_PYTHON) {
    candidates.push({
      label: "GEMINI_SEO_PYTHON",
      exe: stripWrappingQuotes(process.env.GEMINI_SEO_PYTHON),
      args: [],
    });
  }
  candidates.push(
...
function main() {
  ...
  console.error(
    "Gemini SEO hook could not find Python. Tried GEMINI_SEO_PYTHON, py -3, python3, python.",
  );
  process.exit(1);
}
