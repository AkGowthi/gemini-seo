#!/usr/bin/env python3
"""
Backlink API credential management for Gemini SEO.
...
CONFIG_PATH = os.path.expanduser("~/.config/gemini-seo/backlinks-api.json")
CACHE_DIR = os.path.expanduser("~/.cache/gemini-seo/commoncrawl")
...
def load_config() -> dict:
    ...
    Reads ~/.config/gemini-seo/backlinks-api.json first. Any missing fields
...
Free backlink data sources for Gemini SEO. No payment required for any of these.
...
-------------------------------------------------
  ...
  5. Copy your API credentials. Gemini SEO accepts either:
...
def main():
    parser = argparse.ArgumentParser(
        description="Backlink API credential management for Gemini SEO"
    )
    parser.add_argument(
        "--check",
        nargs="?",
