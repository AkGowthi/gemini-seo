...
Config: ~/.config/gemini-seo/dataforseo-costs.json
Ledger: ~/.config/gemini-seo/dataforseo-ledger.json
Set GEMINI_SEO_CONFIG_DIR to point both somewhere else (used by the tests).
...
Security fixes: config path corrected to ~/.config/gemini-seo/
"""
...
# ----- paths -----
# GEMINI_SEO_CONFIG_DIR lets tests (and anyone running an isolated budget) point
# the ledger somewhere other than the real one. Resolved at import.
CONFIG_DIR = Path(
    os.environ.get("GEMINI_SEO_CONFIG_DIR") or Path.home() / ".config" / "gemini-seo"
)
