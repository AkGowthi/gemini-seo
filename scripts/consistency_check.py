...
4. Routing tables in ``skills/seo/SKILL.md`` and ``GEMINI.md`` agree with each
   other and with the skill directories on disk.
5. ``rules/<name>.md`` path mentions exist (``seo-newagent`` doc example whitelisted).
...
def check_skill_dir_script_refs(files, texts):
    """Validate scripts invoked through the agent harness's portable skill-root variable.
    ...
    """
    pat = re.compile(r'\$\{GEMINI_SKILL_DIR\}/scripts/([A-Za-z0-9_]+\.py)\b')
    errors = []
    for f in texts:
        for script in sorted(set(pat.findall(read(f)))):
            ...
            if ext_root:
                ...
                errors.append(f"{f}: GEMINI_SKILL_DIR script used outside a skill layout")
                continue
...
def check_runtime_invocations(texts):
    ...
    carriers = [
        f for f in texts
        if f.startswith(("skills/", "rules/", "extensions/")) and f.endswith(".md")
    ]
    bare = re.compile(
        r"\b(?:python3|python|py\s+-3)\s+[^\n`]*?scripts/[A-Za-z0-9_./-]+\.py"
    )
    raw_path = re.compile(r"(?<![\w/])scripts/([A-Za-z0-9_]+\.py)\b")
    # The canonical form is "${GEMINI_SEO_ROOT}/scripts/gemini-seo" run <script>,
    # so the closing quote may sit between the launcher name and the subcommand.
    runtime = re.compile(
        r"gemini-seo[\"']?\s+run(?:\s+--extension\s+[a-z0-9-]+)?\s+([A-Za-z0-9_-]+\.py)"
    )
    for f in carriers:
        content = read(f)
        ...
        for script in sorted(set(raw_path.findall(content))):
            errors.append(
                ...
                f'use "${{GEMINI_SEO_ROOT}}/scripts/gemini-seo" run {script}'
            )
...
def check_routing(files):
    ...
    tables = {src: {m for m in cmd_pat.findall(read(src)) if m}
              for src in ("skills/seo/SKILL.md", "GEMINI.md")}
    ...
    a, b = tables["skills/seo/SKILL.md"], tables["GEMINI.md"]
    errors = []
    for c in sorted(a - b):
        errors.append(f"routing: `/seo {c}` in orchestrator but not GEMINI.md")
    for c in sorted(b - a):
        errors.append(f"routing: `/seo {c}` in GEMINI.md but not orchestrator")
...
def check_agent_refs(files, texts):
    agents = {os.path.basename(f)[:-3] for f in files
              if f.startswith("rules/") and f.endswith(".md")}
    pat = re.compile(r'rules/([a-z0-9-]+)\.md')
    errors = []
    for f in texts:
        ...
        for m in sorted(set(pat.findall(read(f)))):
            if m not in agents and m not in DOC_EXAMPLE_AGENTS:
                errors.append(f"{f}: dead rule ref rules/{m}.md")
    return errors
