#!/usr/bin/env python3
"""
Google API credential management for Gemini SEO.
...
CONFIG_PATH = os.path.expanduser("~/.config/gemini-seo/google-api.json")
TOKEN_PATH = os.path.expanduser("~/.config/gemini-seo/oauth-token.json")
...
def load_config() -> dict:
    ...
    Reads ~/.config/gemini-seo/google-api.json first. Any missing fields
...
6. CREATE CONFIG FILE
   ...
   Save to ~/.config/gemini-seo/google-api.json:
...
def main():
    parser = argparse.ArgumentParser(
        description="Google API credential management for Gemini SEO"
    )
    parser.add_argument(
        "--check",
        nargs="?",
