...
gemini-seo skills run under Google Gemini / Antigravity, but also Cursor / Cursor Cloud
Agents / Gemini CLI / OpenAI Codex CLI / Cline /
...
def check_one(path: Path) -> list[dict]:
    ...
    # strictly than Gemini CLI-like agent harnesses — warn if there are inline comments
        ...
        if 'version' not in frontmatter["metadata"]:
            findings.append({
                ...
                "message": "metadata block present but no version field; "
                           "gemini-seo's manifest test expects "
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Cross-platform portability lint for gemini-seo SKILL.md files."
    )
