...
def _data_dir(root: Path) -> tuple[Path, str]:
    override = os.environ.get("GEMINI_SEO_DATA_DIR")
    if override:
        return _configured_data_dir(override), "override"
    plugin_data = os.environ.get("GEMINI_SEO_DATA")
    if plugin_data:
    ...
    if _is_plugin(root):
        ...
        return (base / "gemini-seo").resolve(), "plugin-fallback"
    return root, "manual"
    ...
    def __enter__(self) -> "SetupLock":
        ...
        try:
            ...
            if age <= LOCK_STALE_SECONDS:
                raise RuntimeError("another Gemini SEO setup is already running")
...
def command_setup(args: argparse.Namespace) -> int:
    ...
    try:
        with SetupLock(data_dir / ".setup.lock"):
            print("Creating isolated Gemini SEO environment...", flush=True)
        ...
        print(f"Gemini SEO setup failed: {_redact(str(exc))}", file=sys.stderr)
        return 1
    if browser_ready or args.skip_browser:
        print("Gemini SEO runtime is ready.")
        return 0
...
def command_run(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        print(f"Gemini SEO runtime: {exc}", file=sys.stderr)
        return 2
    status = _status(root)
    if not status["ready"]:
        print("Gemini SEO runtime is not ready. Run `/seo setup` and retry.", file=sys.stderr)
        return 3
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="gemini-seo", description="Gemini SEO managed Python runtime")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"Gemini SEO runtime failed: {_redact(str(exc))}", file=sys.stderr)
        return 2
